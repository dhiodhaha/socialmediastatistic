...
import { ChevronDown, CheckCircle2, Layers, Loader2 } from "lucide-react";
...
export function FilterListbox({ value, onChange, options, title, icon: TriggerIcon, prefix, disabled, loading }: FilterListboxProps) {
    return (
                    ...
                    <>
                        <ListboxButton
                            className={cn(
                                ...
                                (disabled || loading) && "opacity-50 cursor-not-allowed active:scale-100 bg-zinc-50 dark:bg-zinc-900"
                            )}
                        >
                            {loading ? (
                            ...
                            <ListboxOptions className="absolute top-12 left-0 z-50 w-72 overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 shadow-xl ring-1 ring-black/5 dark:ring-white/10 focus:outline-none">
                                ...
                                <div className="p-1.5 space-y-1 max-h-80 overflow-y-auto custom-scrollbar">
                                    {options.map((opt, index) => {
                                        const showHeader = opt.group && (index === 0 || options[index - 1].group !== opt.group);
                                        const showDivider = index > 0 && showHeader;

                                        return (
                                            <Fragment key={opt.id}>
                                                {showDivider && <div className="h-px bg-zinc-100 dark:bg-zinc-800 my-1 mx-2" />}
                                                {showHeader && (
