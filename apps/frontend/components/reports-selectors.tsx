...
import { Check, ChevronDown, FileText, ArrowRightLeft, Calendar } from "lucide-react";
...
export function RichSelect({ value, onChange, options, label, icon: TriggerIcon, className }: RichSelectProps) {
    return (
                                ...
                                <ListboxOption
                                    ...
                                    className={({ active, selected }) =>
                                        cn(
                                            ...
                                            active ? "bg-blue-50 dark:bg-blue-900/20" : "text-zinc-900 dark:text-zinc-100",
                                        ...
                                        <>
                                            <div className="flex items-start gap-3">
                                                <div className={cn(
                                                    ...
                                                    selected || active ? "bg-blue-100/50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500"
                                                )}>
                                                    {option.icon ? <option.icon className="h-5 w-5" /> : <FileText className="h-5 w-5" />}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className={cn("block truncate font-semibold text-[13px]", selected ? "text-blue-700 dark:text-blue-400" : "text-zinc-900 dark:text-white")}>
                                                        {option.label}
                                                    </span>
                                                    {option.sublabel && (
                                                        <span className={cn("block truncate text-[11px] font-medium mt-0.5", selected ? "text-blue-600/70 dark:text-blue-400/70" : "text-zinc-400 dark:text-zinc-500")}>
                                                            {option.sublabel}
                                                        </span>
                                                    )}
                                                </div>
